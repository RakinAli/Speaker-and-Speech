...
def enframe(samples, winlen, winshift):
    ...
    enframed_samples[0, start:end] = samples[
        start:end
    ]  # Enframe the first window of samples
    overlap_samples = enframed_samples[
        0, winlen - winshift : winlen
    ]  # Define the first overlap of samples
    start = end  # Update starting pointer
    end += min(winlen - winshift, len(samples) - end)  # Update ending pointer
    while (
        end - start + len(overlap_samples) == enframed_samples.shape[1]
    ):  # While the sample fills the window
        enframed_samples = np.vstack(
            (enframed_samples, np.zeros((1, winlen)))
        )  # Generate new window
        enframed_samples[-1, 0 : len(overlap_samples)] = overlap_samples
        enframed_samples[
            -1, len(overlap_samples) : end - start + len(overlap_samples)
        ] = samples[start:end]
...
def preemp(samples, p=0.97):
    ...
    samples = signal.lfilter(
        [1, -p], [1], samples
    )  # Apply filter to samples enframed matrix
...
def windowing(samples):
    ...
    hamming_window = signal.hamming(
        samples.shape[1], sym=False
    )  # Obtain hamming window according to window shape
...
def dtw(x, y, dist=euclidean):
    ...
    for i in range(1, acc_dist.shape[0]):
        for j in range(1, acc_dist.shape[1]):
            acc_dist[i, j] = lc_dist[i, j] + min(
                acc_dist[i - 1, j], acc_dist[i, j - 1], acc_dist[i - 1, j - 1]
            )
...
def plotPosterior(posterior, component, ut):
    ...
    plt.title(
        "Posterior probability GMM components "
        + str(component)
        + " utterance #"
        + str(ut)
    )
...
def main():
    ...
    mfcc_utterances = []
    mspec_utterances = []
    max_mfcc_length = max(len(mfcc(audio_sample["samples"])[0]) for audio_sample in data)

    for audio_sample in data:
        mfcc_sample, _ = mfcc(audio_sample["samples"])
        # Pad the MFCC sample with zeros if it's not the same length as the longest one
        if len(mfcc_sample) < max_mfcc_length:
            mfcc_sample = np.pad(
                mfcc_sample,
                ((0, max_mfcc_length - len(mfcc_sample)), (0, 0)),
                "constant",
            )
