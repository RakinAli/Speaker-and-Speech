...
def concatTwoHMMs(hmm1, hmm2):
    ...
    """
    hmm1_M = len(hmm1['startprob']) -1 # the number of emitting states in each HMM1 model
    ...
    first_part_transition = np.hstack((hmm1['transmat'][0:hmm1_M,0:hmm1_M], np.outer(hmm1['transmat'][0:hmm1_M,-1], hmm2['startprob']) ))
    second_part_transition = np.hstack((np.zeros((hmm2_start,hmm1_M)) , hmm2['transmat']))
...
def forward(log_emlik, log_startprob, log_transmat):
    ...
    for t in range(1,forward_prob.shape[0]):
        for s in range(forward_prob.shape[1]):
            forward_prob[t][s]=logsumexp(forward_prob[t-1] + log_transmat[:,s]) + log_emlik[t][s]
...
def backward(log_emlik, log_startprob, log_transmat):
    ...
    for t in range((backward_prob.shape[0])-2,-1,-1):
        for s in range(backward_prob.shape[1]):
            backward_prob[t][s] = logsumexp(log_transmat[s,:] + log_emlik[t+1,:] + backward_prob[t+1,:])
...
def viterbi(log_emlik, log_startprob, log_transmat, forceFinalState=True):
    ...
    for t in range(1,viterbi_loglik.shape[0]):
        for s in range(viterbi_loglik.shape[1]): 
             viterbi_loglik[t,s] = np.max(viterbi_loglik[t-1,:] + log_transmat[:,s]) + log_emlik[t,s]
             viterbi_backtrack[t,s] = np.argmax(viterbi_loglik[t-1,:] + log_transmat[:,s]) 
...
def statePosteriors(log_alpha, log_beta):
    ...
    for t in range(log_gamma.shape[0]):
        for s in range(log_gamma.shape[1]):
            log_gamma[t][s] = log_alpha[t][s] + log_beta[t][s] - logsumexp(log_alpha[-1,:])
