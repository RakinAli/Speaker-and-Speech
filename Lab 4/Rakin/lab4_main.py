...
def test(model, device, test_loader, criterion, epoch):
    ...
    with torch.no_grad():
        for I, _data in enumerate(test_loader):
            ...
            for i in range(len(labels)):
                # Most likely, this is the line that is causing the error
            ...
            for j in range(len(decoded_preds)):
                ...
                language_cer.append(cer(text, decoded_targets[j]))
                language_test.append(wer(text, decoded_targets[j]))
    ...
    print("Language model CER: ", sum(language_cer) / len(language_cer))
    print("Language model WER: ", sum(language_test) / len(language_test))
